...
//            if(property.isFunctional) writer.write("\n| rdf:type = owl:FunctionalProperty")
            if(property.equivalentProperties != null) writer.write("\n| owl:equivalentProperty = " + property.equivalentProperties.map(_.name).mkString(","))
            if(property.disjointWithProperties != null) writer.write("\n| owl:propertyDisjointWith  = " + property.disjointWithProperties.map(_.name).mkString(","))
            //TODO- check whether this is correct
            if(property.rdfTypes != null) writer.write("\n| rdf:type = " + property.rdfTypes.map(_.name).mkString(","))
            writer.write("\n}}")
